...
func _ready() -> void:
	...
	check(not (wing.task_lights["security"] as Light3D).visible, "security work lamp loses normal circuit power")
	cctv_breaker.interact(player)
	check((wing.task_lights["security"] as Light3D).visible, "security work lamp recovers with circuit power")
	camera.interact(player)
