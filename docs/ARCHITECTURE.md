...
`vertical_slice.gd` owns this slice's physical flow and presentation. `HabitDetector` maps meaningful events into `BehaviourMemory`; `AdaptiveEventDirector` scores a small set of slice-specific candidates in `SliceEventOptions` under trust, confidence, suspicion and cooldown constraints. `OtherPlayerAgent` schedules contextual actions using complexity, behavioural hesitation, confidence, simulated activity, and deterministic variation. `PredictionSystem` issues, resolves, expires, and serializes predictions. `TerminalTypewriter` handles diegetic text reveal. Reusable material resources and a procedural detail builder keep geometry text-based.
...
The director is a reusable scoring/pacing component, not a general authored-event content pipeline. Its current candidate catalog covers only the corridor assist, delayed power acknowledgement, and reassurance. The physical story beats remain scene-owned.
