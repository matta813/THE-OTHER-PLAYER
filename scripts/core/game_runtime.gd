extends Node
var behaviour: BehaviourRecorder; var trust := TrustModel.new(); var predictions := PredictionSystem.new(); var other_player: OtherPlayerAgent; var habits := BehaviourMemory.new(); var detector := HabitDetector.new(habits); var expectations := ExpectationModel.new(); var suspicion := SuspicionModel.new(); var director := AdaptiveEventDirector.new(); var trust_strategy := TrustStrategy.new(); var facility: Dictionary = {}; var story_stage := 0; var playtime := 0.0
func _ready() -> void:
...
func reset_state() -> void:
	if behaviour:
		behaviour.event_recorded.disconnect(_on_behaviour_event)
		remove_child(behaviour)
		behaviour.queue_free()
	if other_player:
		remove_child(other_player)
		other_player.queue_free()
	facility.clear(); story_stage = 0; playtime = 0.0
	trust = TrustModel.new(); predictions = PredictionSystem.new(); habits = BehaviourMemory.new(); detector = HabitDetector.new(habits); expectations = ExpectationModel.new(); suspicion = SuspicionModel.new(); director = AdaptiveEventDirector.new(); trust_strategy = TrustStrategy.new()
	behaviour = BehaviourRecorder.new(); behaviour.name = "BehaviourRecorder"; add_child(behaviour); behaviour.event_recorded.connect(_on_behaviour_event)
	other_player = OtherPlayerAgent.new(); other_player.name = "OtherPlayerAgent"; add_child(other_player)
