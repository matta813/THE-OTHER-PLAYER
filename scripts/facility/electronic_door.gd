...
var opened := false; var target_angle := 0.0; var powered := true
...
func prompt_text(actor: Node) -> String: return "NO POWER" if not powered else ("LOCKED // REMOTE CONTROL" if locked else super.prompt_text(actor))
func _perform_interaction(_actor: Node) -> void:
	if not powered:
		GameRuntime.behaviour.record(&"unavailable_interaction", global_position, stable_id, &"no_power"); return
	if locked:
...
func remote_action(action: StringName, _payload: Dictionary = {}) -> bool:
	...
	if action == &"close": opened = false; target_angle = 0.0; set_process(true); return true
	return false
func state_dict() -> Dictionary: return {"locked": locked, "opened": opened, "angle": target_angle, "powered": powered}
func load_state(data: Dictionary) -> void: locked = data.get("locked", true); opened = data.get("opened", false); powered = data.get("powered", true); target_angle = data.get("angle", open_angle if opened else 0.0); rotation.y = deg_to_rad(target_angle); set_process(false); state_changed.emit()
