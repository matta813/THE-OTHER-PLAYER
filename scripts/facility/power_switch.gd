...
func _perform_interaction(_actor: Node) -> void:
	...
	powered = true; rotation.z = deg_to_rad(22); power_changed.emit(true); state_changed.emit()
	var response := Time.get_ticks_msec() / 1000.0 - float(get_meta("requested_at", Time.get_ticks_msec() / 1000.0)); GameRuntime.behaviour.record(&"instruction_completed", global_position, stable_id, &"power_request", response); GameRuntime.trust.player_response(response, true); GameRuntime.predictions.observe(&"interact", stable_id, Time.get_ticks_msec() / 1000.0)
