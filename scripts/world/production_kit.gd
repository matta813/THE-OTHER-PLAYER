...
static func add_visual(parent: Node3D, name: String, position: Vector3 = Vector3.ZERO, rotation: Vector3 = Vector3.ZERO, scale: Vector3 = Vector3.ONE) -> Node3D:
	var category := PROPS if name in ["electrical_cabinet", "breaker_panel", "equipment_rack", "terminal_housing", "security_camera_housing", "security_console", "transfer_hatch", "airlock_door", "generator_control_unit", "generator_unit", "airlock_cycle_panel"] else ENVIRONMENT
